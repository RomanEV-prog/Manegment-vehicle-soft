...
ITEM_KEYS = (
    "sw_version", "sw_file_name", "sw_file_sha256", "sw_config_version",
    "sw_config_file_name", "sw_config_sha256", "compatible_hardware",
)
...
async def build_snapshot(db: AsyncSession, vehicle: Vehicle, installed: dict[str, str]) -> dict:
    ...
    if installed:
        ...
        for b in sorted(baselines, key=lambda b: b.rxswin_ref.rxswin):
            rxswins.append({
                ...
                "items": [
                    {"ecu": i.ecu.ecu_name, "ecu_id": str(i.ecu_id), "part_number": i.ecu.eversum_part_number,
    ...
    ecus = [
        {"ecu": e.ecu_name, "ecu_id": str(e.id), "part_number": e.eversum_part_number,
         "serial_number": ve.serial_number, "hardware_version": ve.hardware_version, "batch_number": ve.batch_number}
...
async def record_configuration(
    ...
    *,
    ...
    if config_id is None:
        n = await db.scalar(select(func.count()).select_from(VehicleConfiguration)
                            .where(VehicleConfiguration.vehicle_id == vehicle.id))
        config_id = f"LKC-{vehicle.vin}-{n + 1:03d}"
    previous = await current_configuration(db, vehicle.id)
    cfg = VehicleConfiguration(
        organization_id=vehicle.organization_id, vehicle_id=vehicle.id, config_type=config_type,
        config_id=config_id, snapshot=await build_snapshot(db, vehicle, installed),
        system_schemes_baseline=system_schemes_baseline or (previous.system_schemes_baseline if previous else None),
        vv_status=vv_status, erp_work_order=erp_work_order or (previous.erp_work_order if previous else None),
        software_update_id=software_update_id, locked=True, reason=reason, created_by=user_id,
    )
    db.add(cfg)
    await db.flush()
    return cfg
